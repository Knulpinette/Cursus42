...
** 				  it apparently prints nothing at all. On linux only.
**				  Go figure.
**				-> cf handle_precision (in the if, add at the beginning)
...
void	handle_precision(char **conv, t_flags **flags, int *len)
{
	char	*temp;
...
char	*convert_s(va_list arg, t_flags *flags)
{
	...
	len = ft_strlen(conv);
	handle_precision(&conv, &flags, &len);
	if (flags->width > len)
	{
		flags->zero = none;
